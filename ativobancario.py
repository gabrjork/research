...
def aggregate_bank_assets(folder_path, output_path, target_codes, code_col, asset_col, date_col, max_rows)
    ...
    if not csv_files
        print(
            fERRO Nenhum arquivo CSV encontrado na pasta especificada '{os.path.abspath(folder_path)}')
        return

    print(
        fEncontrados {len(csv_files)} arquivos CSV. Processando (lendo no máx. {max_rows} linhas por arquivo)...)

    for file_path in csv_files
        filename = os.path.basename(file_path)
        print(fProcessando arquivo {filename}...)
    ...
    print(nConsolidando resultados...)
    ...
    try
        ...
        print(
            fAviso Não foi possível analisar ou ordenar as datas consistentemente. Erro {e}. A saída não será ordenada.)

    # Salva o resumo consolidado em um arquivo Excel
    try
        print(fSalvando resumo consolidado em {output_path})
        summary_df.to_excel(output_path, index=False, engine='openpyxl')
        print(Consolidação completa! Arquivo de saída criado.)
    except Exception as e
        print(
            fERRO Não foi possível salvar o arquivo Excel de saída '{output_path}'. Motivo {e})
...
if __name__ == __main__
    if not os.path.isdir(INPUT_FOLDER_PATH)
        print(
            fERRO Pasta de entrada não encontrada em '{os.path.abspath(INPUT_FOLDER_PATH)}'. Por favor, verifique a variável 'INPUT_FOLDER_PATH'.)
    else
